...
class Iourt43Parser(Iourt41Parser):
    ...
    _lineFormats = (
        ...
        #ClientDead: 1 2 452.554 785.544 451455
        re.compile(r'^(?P<action>ClientDead):\s(?P<cid>[0-9]+)\s(?P<tid>[0-9]+)\s(?P<x>-?\d+(?:\.\d+)?)\s(?P<y>-?\d+(?:\.\d+)?)\s(?P<time>[0-9]+)$', re.IGNORECASE),
    ...
    def OnClientdead(self, action, data, match=None):
        ...
        if not client:
            self.debug('No client found')
            return None
        attacker = self.clients.getByCID(match.group('tid'))
        if not attacker:
            self.debug('No attacker')
            return None
        return Event(self.EVT_CLIENT_DEAD, client=client, target=attacker, data=dict(x=x, y=y , time=time))
